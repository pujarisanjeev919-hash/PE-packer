...
// load a PE in memory, returna the entry points
...
void* load_PE(char* PE_data){
    ...
    DWORD size_of_header = p_NT_HDR->OptionalHeader.SizeOfHeaders;
    ...
    /* Maps PE sections in Memory */
    memcpy(ImageBase,PE_data,size_of_header);

    // Sections headers starts right after the IMAGE_NT_HEADERS struct, so we do some pointer arithmetic-fU here.
    IMAGE_SECTION_HEADER* sections = (IMAGE_SECTION_HEADER*) (p_NT_HDR + 1);

    // For each sections
    for(int i=0; i<p_NT_HDR->FileHeader.NumberOfSections; ++i){
    // calculate the VA we need to copy the content, from RVA
    // section[i].VirtualAddress is a RVA, mind it
    char* dest = ImageBase + sections[i].VirtualAddress;

    // check if there us Raw data to copy
    ...
    // Load the address of the import descriptor arry
    ...
    for(int i=0; import_descriptors[i].OriginalFirstThunk != 0; ++i){
        ...
        // the lookup table points to function name or ordinals => it is the IDT
        ...
        // but we put the address of the loaded function inside => that's the IAT
        ...
        for(int i=0; lookup_table[i].u1.AddressOfData != 0; ++i){
            ...
            // check the lookup table for the address if the function name to import
            ...
            if((lookup_addr & IMAGE_ORDINAL_FLAG) == 0){ // if first bit is not 1
                ...
                // this struct points to the ascii functin name
            ...
            // change the IAT, and put the pointer the fuction address inside.
